 ...
 * 2. 使用corpid/corpsecret获取token。并调用用户传入的保存token方法保存
 *
 * Tips:
 *
 ...
 * ```
 * var api = new API('corpid', 'corpsecret', function (callback) {
 *   // 传入一个获取全局token的方法
...
/*!
 * 根据创建API时传入的corpid和corpsecret获取access token
 * 进行后续所有API调用时，需要先获取access token
 * 详细请看：<http://qydev.weixin.qq.com/wiki/index.php?title=主动调用>
 *
 * 应用开发者无需直接调用本API。
 *
 * Examples:
